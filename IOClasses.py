...
# jdenhaan, 2022-Dec-11, added functions to allow interaction with track data
#------------------------------------------#
                ...
                for disc in lst_Inventory:
                    ...
                    for Single in tracks:
                        if Single is not None:
                            single_info = '{},{}'.format(disk_id, Single.get_record())
                            file.write(single_info)
            # TODONE add code to save track data to file
        except Exception as e:
            print('There was a general error with save_inventory!', e, e.__doc__, type(e), sep='\n')
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open (file_name_track, 'r') as file:
                for line in file:
                    data= line.strip().split(',')
                    Album = PC.DataProcessor.select_cd(lst_Inventory, int(data[0]))
                    Single = DC.Track(int(data[1]), data[2], data[3])
                    Album.add_track(Single)
            # TODONE add code to load track data
        except Exception as e:
            print('There was a general error with load_inventory!', e, e.__doc__, type(e), sep='\n')
