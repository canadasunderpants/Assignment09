...
# jdenhaan, 2022-Dec0-11, added function code as required 
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            print('This ID is not an integer')
            print(e.__doc__)
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception ('This CD index does not exist!')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # TODONE add code as required
        track_position, track_title, track_length = track_info
        try:
            track_position = int(track_position)
        except:
            raise Exception ('Track Position must be an integer')
        track = DC.Track(track_position, track_title, track_length)
        cd.add_track(track)